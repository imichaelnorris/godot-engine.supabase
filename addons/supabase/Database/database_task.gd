...
func _setup(query: SupabaseQuery, code: int, endpoint: String, headers: PackedStringArray, payload: String=""):
	_query = query
	_code = code
	_endpoint = endpoint
	_headers = headers
...
func push_request(httprequest: HTTPRequest) -> void:
	...
	httprequest.connect("request_completed", _on_task_completed)
