...
# renamed to _rpc because rpc has a different purpose in Godot.
func _rpc(function_name: String, arguments: Dictionary={}, supabase_query: SupabaseQuery=null) -> DatabaseTask:
	_bearer = get_parent().auth._bearer
...
func _process_task(task: DatabaseTask) -> void:
	...
	task.connect("completed", _on_task_completed)
